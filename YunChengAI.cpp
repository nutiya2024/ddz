...
bool cardLessThan(const int a, const int b) {
    int res = getCardValueInt(a) - getCardValueInt(b);
    if (res == 0) {
        res = a - b;
    }
    return res < 0;
...
// 获得最大牌值(比如顺子：5，6，7，8，9，T, TopValue:T)
...
std::vector<int> LordCards::removeSubset(const std::vector<int> & subset) {
    ...
    for (unsigned s=0; s<subset.size(); ++s) {
        bool removed = false;
        int obj = subset[s];

        for (unsigned k=0; k<theCards.size(); ++k) {
            if (theCards[k] == obj) {
...
int LordCards::scanToTable(){
    ...
    for (unsigned i=0; i<theCards.size(); i++) {
...
OneHand LordCards::calcPowerByRemoveNode(const AINode & node) {
    ...
    OneHand hand = other.calcPowerValue(false);
    return hand;
}
