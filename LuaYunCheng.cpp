...
int luaopen_YunCheng(lua_State*);
...
// #pragma mark - Game Environments
    ...
    int lgetHandCards(lua_State* L) {
        ...
        lua_newtable(L);
        for (unsigned i=0; i < one.theCards.size(); ++i) {
    ...
    int lsetHandCount(lua_State* L) {
        ...
        int seatId   = (int)lua_tointeger(L, 2);
        int cnt_num = (int)lua_tointeger(L, 3);

        LordCards & one = *game->seatHands[seatId];
        one.num = cnt_num;
        //game->seatHands[seatId].num = cnt_num;
    ...
    int do_robotFirstPlay(lua_State* L, struct YunChengGame * game) {
        ...
        //printf("direct    : %s\n\n%d\n\n", game->debugSnakeInfo(lastNode.cards).c_str(), one.num);

        if (!nodes.empty()) {

            lastNode = nodes[0];
            if(game->curSeatId != game->masterSeatId && lastNode.cards.size() != one.num) 
            {
                //什么
                LordCards & master = *game->seatHands[game->masterSeatId];
               // printf("master    : %d\n\n", master.num);
                if( master.num <= 2) {
                    for (unsigned i=0; i < nodes.size(); ++i) {
                        if((master.num == 1 && nodes[i].cards.size() > 1) || (master.num == 2 && nodes[i].cards.size() != 2) )
                        {
                            lastNode = nodes[i];
                            break;
                        }
...
// #pragma mark - Library Settings
    ...
    {
        // 环境设置
        {"updateSeats",             lupdateSeats},  //更新玩家信息
        {"addHistCards",            laddHistCards}, //历史出牌,好像没用到
        {"setHandCards",            lsetHandCards}, //设置手牌
        {"getHandCards",            lgetHandCards}, //获得手牌
        {"setHandCount",            lsetHandCount}, //手牌设置，只知道别人的牌数
    ...
    int luaopen_YunCheng (lua_State* L)
    {
        ...
        luaL_setfuncs(L,yunchengMeta,0);
